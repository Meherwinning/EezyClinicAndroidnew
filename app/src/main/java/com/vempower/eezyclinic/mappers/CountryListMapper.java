...
public class CountryListMapper extends  AbstractMapper  implements Callback<CountryListAPI> {
    ...
    public void setOnCountryListListener(CountryListListener listener) {
        if (listener == null) {
            Log.i(MyApplication.getCurrentActivityContext().getClass().getName(), "Invalid CountryListListener instance.");
            return;
    ...
    private void init() {
        ...
        if (!Utils.isNetworkAvailable(MyApplication
            ...
            if (listener != null) {
                listener.getCountryListAPI(null, Utils.getStringFromResources(R.string.network_not_available_lbl));
            }
            return;
        }
    ...
    public void onResponse(Response<CountryListAPI> response, Retrofit retrofit) {
        ...
        getMyResponse(response, new MyResponse<CountryListAPI>() {
            @Override
            public void getMyResponse(CountryListAPI responseBody, String errorMsg) {
                listener.getCountryListAPI(responseBody,errorMsg);
            }
        });
